...
import type { AllRole, CmsUser } from '@/lib/validations/user'
import type { User as SupabaseUser } from '@supabase/supabase-js'

/** Transform Supabase Auth user to CmsUser (snake_case → camelCase) */
export function transformAuthUser(authUser: SupabaseUser): CmsUser {
  return {
    id: authUser.id,
    email: authUser.email ?? '',
    role: (authUser.app_metadata?.role ?? 'user') as AllRole,
