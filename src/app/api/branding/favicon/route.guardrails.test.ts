/**
 * Branding Favicon API Route Guardrail Tests (POST/DELETE /api/branding/favicon)
 */
...
describe('POST /api/branding/favicon Guardrails', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('P0: Auth Guard', () => {
    it('[P0] MUST call requireApiAuth with "admin"', async () => {
      ...
      vi.mocked(uploadBrandingAsset).mockResolvedValue('https://example.com/branding/favicon/new.png')
  ...
  describe('P1: Validation', () => {
    ...
    it('[P1] File too large (>64KB) MUST return 400', async () => {
      vi.mocked(requireApiAuth).mockResolvedValue(createMockAuth())

      const response = await POST(createUploadRequest({ size: 100 * 1024 }))
      const body = await response.json()
    ...
    it('[P1] Invalid MIME type MUST return 400', async () => {
      ...
      const response = await POST(createUploadRequest({ type: 'image/jpeg' }))
      const body = await response.json()
    ...
    it('[P1] Valid MIME types MUST be accepted', async () => {
      for (const type of ['image/png', 'image/svg+xml', 'image/x-icon']) {
        vi.clearAllMocks()
        vi.mocked(requireApiAuth).mockResolvedValue(createMockAuth())
        vi.mocked(getLandingPageContent).mockResolvedValue(createMockLandingPageContent())
        vi.mocked(uploadBrandingAsset).mockResolvedValue('https://example.com/favicon.png')

        const response = await POST(createUploadRequest({ type }))
  ...
  describe('P1: Success', () => {
    it('[P1] Valid upload MUST return new URL', async () => {
      ...
      expect(response.status).toBe(200)
      expect(body.data.faviconUrl).toBe('https://example.com/branding/favicon/new.png')
      expect(updateSectionContent).toHaveBeenCalled()
    })
  })
})

describe('DELETE /api/branding/favicon Guardrails', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(getLandingPageContent).mockResolvedValue(
      createMockLandingPageContent({
        theme: { colorScheme: 'dxt-default', font: 'nunito', logoUrl: null, faviconUrl: 'https://example.com/favicon.png' },
      }),
    )
  })
