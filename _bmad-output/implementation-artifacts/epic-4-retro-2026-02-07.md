...
|---|------|----------|-------|--------|
| D1 | Fix flaky `preview/route.test.ts` timeout in full suite | MEDIUM | Converted `await import()` to static imports in 2 files | ✅ Fixed |
...
| BF4 | Admin skeleton loading ไม่ consistent — ใช้ปนกัน 3 แบบ (LoadingSpinner / animate-shimmer / animate-pulse+slate) ตำแหน่งเบี้ยว (loading.tsx ไม่มี page wrapper) → Standardize ทุกหน้าใช้ `animate-pulse bg-muted` + แยก Skeleton component (Suspense fallback) กับ loading.tsx (route-level + page wrapper) + แก้ button skeleton ให้ตรง h-11 (44px touch target) + เพิ่ม bg-card บน card skeleton + ลบ orphaned animate-shimmer CSS | ✅ Fixed |
...
| Area | Status |
|------|--------|
| Testing & Quality | ✅ 1337 tests across 115 files, all passing, 0 regressions |
...
2. **Apply team agreements** from this retrospective immediately
