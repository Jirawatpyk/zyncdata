...
|---|--------|-------|-----------------|
| P1 | Add post-install verification for shadcn: after any `npx shadcn@latest add`, run lint + check modified files for `dark:` classes and verify `min-h-11` on Button/Input | Dev (Amelia) | Zero shadcn-related regressions in Epic 4 code reviews |
...
| P3 | Add `database.ts` sanity check after `supabase gen types`: verify line 1 is valid TypeScript | Dev (Amelia) | Zero database.ts corruption issues |
...
|---|------|-------|----------|-------|
| D1 | Bundle size at 397.81 KB (over 350 KB limit) | Dev (Amelia) + Jiraw | **HIGH — BLOCKER** | Must audit, code-split, or revise limit before Epic 4 |
...
| # | Item | Owner |
|---|------|-------|
| DOC1 | Document shadcn post-install verification checklist | Dev (Amelia) |
...
**Epic Update Required:** YES — Bundle size strategy required

During Epic 3, the bundle reached 397.81 KB (over 350 KB limit). Epic 4 requires TipTap WYSIWYG editor which adds 50-100 KB. The current plan assumes TipTap can be added without friction, but CI will reject builds over 350 KB.

**Required before Epic 4:**
1. Resolve bundle size strategy (raise limit with justification, code-split to admin routes, or both)
2. Complete TipTap spike (bundle measurement, SSR/RSC compatibility, testing approach)
3. Update Epic 4 stories if alternative to TipTap is chosen or if bundle-related constraints affect story scope
...
| Task | Owner | Notes |
|------|-------|-------|
| Bundle size audit + strategy decision | Dev (Amelia) + Jiraw | Raise limit? Code-split? Audit deps? |
...
| Technical Health | ⚠️ Bundle at 397 KB — over limit but functional |
| Unresolved Blockers | ⚠️ Bundle strategy + TipTap spike needed before Epic 4 |

**Verdict:** Epic 3 is fully complete. Team has 2 critical path items to resolve before Epic 4 can begin.

---

## Next Steps

1. **Resolve bundle size strategy** — Jiraw + Dev (Amelia) to decide: raise limit, code-split, or both
2. **Complete TipTap spike** — Dev (Amelia) to prototype, measure bundle, test compatibility
3. **Begin Epic 4** with Story 4.1 once critical path items are clear
