...
- [ ] Research Next.js 16 `proxy.ts` route protection patterns — Owner: Dev (Amelia)
- [ ] Verify Supabase Auth MFA API current behavior — Owner: Dev (Amelia)
- [ ] Update architecture.md technology versions — Owner: Architect (Winston)

### Parallel (can happen during early stories)

- [ ] Establish auth testing patterns (mock Supabase Auth, test MFA) — Owner: QA (Dana)
- [ ] Provision Upstash Redis for rate limiting — Owner: Jiraw (Project Lead)

### Nice-to-have

- [ ] Evaluate Sentry SDK tree-shaking to recover bundle budget — Owner: Dev (Amelia)
